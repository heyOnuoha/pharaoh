...
Future<void> testBasicRateLimit(HttpClient client) async {
  ...
  for (int i = 1; i <= 5; i++) {
    try {
      final request = await client.getUrl(Uri.parse('http://localhost:3000/api/public/status'));
      final response = await request.close();
      final body = await response.transform(utf8.decoder).join();
      
      print('Request $i: ${response.statusCode} - ${response.headers['ratelimit-remaining']?[0] ?? 'N/A'} remaining');
...
Future<void> testCustomKeyGeneration(HttpClient client) async {
  ...
  for (final userId in userIds) {
    try {
      final request = await client.postUrl(Uri.parse('http://localhost:3000/api/sensitive/data'));
...
Future<void> testSkipFunctionality(HttpClient client) async {
  ...
  try {
    final request1 = await client.postUrl(Uri.parse('http://localhost:3000/api/sensitive/data'));
    ...
    print('Regular user: ${response1.statusCode} - ${response1.headers['ratelimit-remaining']?[0] ?? 'N/A'} remaining');
  } catch (e) {
  ...
  try {
    final request2 = await client.postUrl(Uri.parse('http://localhost:3000/api/sensitive/data'));
...
Future<void> testSlidingWindow(HttpClient client) async {
  ...
  for (int i = 1; i <= 3; i++) {
    try {
      final request = await client.postUrl(Uri.parse('http://localhost:3000/api/uploads/file'));
      request.headers.set('content-type', 'application/json');
